...
# Sequential History Settings
USE_SEQUENTIAL_HISTORY = True  # Set to False to use simple pooling
LSTM_UNITS = 64
HISTORY_ATTENTION = True  # Use attention over history
...
print(f"Sequential History Processing: {USE_SEQUENTIAL_HISTORY}")
print(f"History Attention: {HISTORY_ATTENTION}")
print(DASHLINE)
...
def _pad_or_truncate(items, max_len=MAX_HISTORY):
    """Keep the MOST RECENT items (end of list) since we know chronological order"""
    items = list(items)
    # Take last N items (most recent)
    items = items[-max_len:] if len(items) > max_len else items
    # Pad at the beginning (older positions) if needed
    if len(items) < max_len:
        items = [""] * (max_len - len(items)) + items
    return items

def _remove_target_from_history(row):
    """
    Remove the target article from purchase history to prevent leakage.
    IMPORTANT: We keep the chronological order!
    """
    history = _ensure_list(row["purchased_articles"]).copy()
    target = row["article_id"]
    
    # Remove target while preserving order
    history = [item for item in history if item != target]
    
    # Pad/truncate while keeping most recent items
    return _pad_or_truncate(history)

def _get_position_in_history(row):
    """
    Get the position of the target article in the original purchase sequence.
    This tells us how recent the purchase is.
    Returns: position from end (1 = most recent purchase, 2 = second most recent, etc.)
    """
    history = _ensure_list(row["purchased_articles"])
    target = row["article_id"]
    
    try:
        # Find position from the end (reverse index)
        reversed_history = list(reversed(history))
        position_from_end = reversed_history.index(target) + 1
        return position_from_end
    except ValueError:
        # Target not in history (shouldn't happen)
...
# Create history without target
interactions["history_article_ids"] = interactions.apply(_remove_target_from_history, axis=1)

# NEW: Add recency information
interactions["target_recency"] = interactions.apply(_get_position_in_history, axis=1)
interactions["log_recency"] = np.log1p(interactions["target_recency"])  # Log transform for better distribution

print(f"Average target recency: {interactions['target_recency'].mean():.2f}")
print(f"Median target recency: {interactions['target_recency'].median():.2f}")
...
print("Interaction dataframe is ready with sequential purchase history!")
print("IMPORTANT: Target articles removed from history, chronological order preserved")
print(DASHLINE)
...
def df_to_dataset(df):
    ...
    return tf.data.Dataset.from_tensor_slices({
        ...
        "log_recency": df["log_recency"].astype("float32").values,
...
class SequentialHistoryEncoder(tf.keras.layers.Layer):
    """
    Encodes purchase history using LSTM to capture sequential patterns.
    Uses attention mechanism to focus on relevant past purchases.
    """
    def __init__(self, article_embedding, use_attention=True, **kwargs):
        ...
        if use_attention:
            ...
            self.query_dense = tf.keras.layers.Dense(LSTM_UNITS)
        else:
            self.pooling = tf.keras.layers.GlobalAveragePooling1D()
    
    def call(self, history_ids, mask, training=False):
        # Get embeddings for history
        history_emb = self.article_embedding(history_ids)
        
        # Process sequence with LSTM
        lstm_out = self.lstm(history_emb, mask=mask, training=training)
        
        if self.use_attention:
            # Use last LSTM state as query for attention
            query = lstm_out[:, -1:, :]
            ...
            # Simple average pooling with mask
            mask_expanded = tf.cast(mask, tf.float32)[:, :, tf.newaxis]
            masked_out = lstm_out * mask_expanded
            pooled = tf.reduce_sum(masked_out, axis=1) / (tf.reduce_sum(mask_expanded, axis=1) + 1e-9)
            return pooled
...
class CustomerModel(tf.keras.Model):
    def __init__(self, article_lookup, article_embedding):
        ...
        if USE_SEQUENTIAL_HISTORY:
            self.history_encoder = SequentialHistoryEncoder(
                article_embedding, 
                use_attention=HISTORY_ATTENTION
            )
        else:
    ...
    def call(self, features, training=False):
        ...
        log_recency = tf.expand_dims(features["log_recency"], -1)
        
        # Create mask for valid history items (non-empty strings)
        mask = tf.not_equal(features["history_article_ids"], "")
        history_ids = self.article_history_lookup(features["history_article_ids"])
        
        if USE_SEQUENTIAL_HISTORY:
            history_encoded = self.history_encoder(history_ids, mask, training=training)
        else:
            # Simple pooling fallback
            history_emb = self.article_history_embedding(history_ids)
            mask_expanded = tf.cast(mask, tf.float32)[:, :, tf.newaxis]
            history_emb = history_emb * mask_expanded
            history_encoded = tf.reduce_sum(history_emb, axis=1) / (tf.reduce_sum(mask_expanded, axis=1) + 1e-9)
...
with mlflow.start_run() as run:
    ...
    # Build models by calling with real tensors
    model.customer_model({
        ...
        "log_recency": tf.constant([0.0], dtype=tf.float32),
    })
    ...
    mlflow.log_params({
        ...
        "sequential_history": USE_SEQUENTIAL_HISTORY,
        "history_attention": HISTORY_ATTENTION,
        "lstm_units": LSTM_UNITS if USE_SEQUENTIAL_HISTORY else None,
    })
