...
print("Starting Job: creating Stores Collection in Mongo")
print(dashline)

print("Loading dataset...")
stores_csv = "/data/processed/stores_df.csv"
chunksize = 50000
stores_df = pd.read_csv(stores_csv, chunksize=chunksize)
print("Successful! Now moving to writing operation...")
print(dashline)

print("Initiating collection in Mongo")
stores_collection = db["stores"]
stores_collection.create_index("store_id", unique=True)
print(dashline)
...
with tqdm() as pbar:
    for chunk in stores_df:
        docs = [row_to_doc(r) for r in chunk.to_dict(orient="records")]
        if not docs:
            continue
        operations = [UpdateOne({"store_id": d["storeCode"]}, {"$set": d}, upsert=True) for d in docs]
        result = stores_collection.bulk_write(operations, ordered=False)
